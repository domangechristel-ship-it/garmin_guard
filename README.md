...
Personal athletic performance dashboard built on top of Garmin Connect data exports. Parses raw JSON exports locally, computes training load metrics, and serves them through a FastAPI backend with a Streamlit dashboard — with an optional sync to BigQuery for cloud deployment.
...
```
Garmin DI_CONNECT export          Strava API
         │                              │
         ▼                              ▼
  garmin_parser.py            strava_fetch.py
  wellness_parser.py                    │
         │                              │
         └──────────────┬───────────────┘
                        ▼
...
| `strava_fetch.py` | Strava API | merged into activities |

**Wellness signals parsed:** sleep score & durations (deep / light / REM / awake), menstrual cycle phases, daily symptoms & moods, weight, resting HR, VO2max (biometric), biological age, BMI, HRV (RMSSD & SDRR).
...
- Python 3.12
- A Garmin Connect data export placed under `data/raw/DI_CONNECT/`
- (Optional) Strava API credentials in `.env` for Strava sync
...
|---|---|
| `make garmin_update` | Parse activities + wellness CSVs from raw export |
| `make load_duckdb` | Load processed CSVs into `data/garmin_guard.duckdb` |
| `make garmin_full_update` | Parse + load DuckDB (no Strava, no BigQuery) |
| `make fetch_new_data` | Full pipeline: parse · sync Strava · load DuckDB · export BigQuery |
...
Three tabs, all filtered by date range and sport type from the sidebar:
...
**Wellness** — sleep score with 7-day rolling average; sleep duration breakdown (deep / REM / light / awake); weight trend. All charts show menstrual cycle phases as coloured vertical bands.
...
- **Ingestion** — Python, pandas, garminconnect, stravalib
